  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
        ...
        <button onClick={handleExport} disabled={initialTransactions.length === 0 || !hasFullAccess} title={!hasFullAccess ? 'Berlangganan untuk export Excel' : undefined} className="btn-primary text-xs px-3 py-2 flex items-center gap-1 font-medium">
          <Download size={14} />
          Export ke Excel
        </button>
      </div>
                  ...
                  <tr key={t.id} className="border-t" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <div className="flex gap-2 justify-end">
                        <button
                          onClick={() => hasFullAccess && startEdit(t)}
                          disabled={!hasFullAccess}
                          title={!hasFullAccess ? 'Berlangganan untuk edit transaksi' : undefined}
                          style={{ color: hasFullAccess ? 'var(--ink-soft)' : '#c9c7c0' }}
                        >
                          <Pencil size={14} />
                        </button>
                        <button
                          onClick={() => hasFullAccess && handleDelete(t.id)}
                          disabled={!hasFullAccess}
                          title={!hasFullAccess ? 'Berlangganan untuk hapus transaksi' : undefined}
                          style={{ color: hasFullAccess ? 'var(--danger)' : '#c9c7c0' }}
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
